...
let rec compileExp  (e      : TypedExp)
  ...
  | Constant (BoolVal p, _) ->
      ...
      // If ture then 1 else 0 basically.
  ...
  *)
  ...
  | Divide (e1, e2, pos) ->
      let t1: reg = newReg "divide_L"
      let t2 = newReg "divide_R"
      let code1 = compileExp e1 vtable t1
      let code2 = compileExp e2 vtable t2
      let notZero = newLab "NotZero"
      let zero = newLab "zero"
      let checkZero = [BNE(t2,Rzero, notZero)
      ;LA(Ra1, "m.DivZero")
      ;J "p.RuntimeError"
      ;J zero
      ;LABEL (notZero)
      ]
      code1 @ code2 @ checkZero  @[DIV (place,t1,t2); LABEL (zero)]
      
  | Not (e1, pos) ->
      let t1: reg = newReg "not_exp"
      let code1 = compileExp e1 vtable t1
      let notCode = [XORI (place, t1, 1)]
      code1 @ notCode

  | Negate (e1, pos) ->
      let t1 = newReg "neg_exp"
      let code1 = compileExp e1 vtable t1
      let neg = [SUB (place,Rzero,t1)]
      code1@neg
  ...
  *)
  (*Shortcircuit states if the first result is false for AND then the statement is false and the
  opposite is the case for OR hence check the first statement if the first statement can 
  conclude the entire statement skip the second statement and return the result.*)
  | And (e1, e2, pos) ->
      let t1 = newReg "and_L"
      let code1 = compileExp e1 vtable t1
      let isTrue = newLab "isTrue"
      let isFalse = newLab "isFalse"
      let shortcircuit = [BNE(t1,Rzero, isTrue)
      ;LI (place, 0)
      ;J isFalse
      ;LABEL (isTrue)
      ]
      let t2 = newReg "and_R"
      let code2 = compileExp e2 vtable t2
      code1 @ shortcircuit @ code2 @ [AND (place,t1,t2); LABEL isFalse]
  ...
  | Or (e1, e2, pos) ->
      ...
      let isTrue = newLab "isTrue"
      let isFalse = newLab "isFalse"
      let shortcircuit = [BEQ(t1,Rzero, isFalse)
      ;LI (place, 1)
      ;J isTrue
      ;LABEL (isFalse)
      ]
      let t2 = newReg "or_R"
      let code2 = compileExp e2 vtable t2
      code1 @ shortcircuit @ code2 @ [OR (place,t1,t2); LABEL isTrue]
  ...
  *)
  //Slet ikke done so far hare jeg bare copy pasted Iota
  | Replicate (n,a, ret_type, pos) ->
      let size_reg = newReg "size"
      let a_reg = newReg "a"
      let n_code = compileExp n vtable size_reg
      (* size_reg is now the integer n. *)
      let a_code = compileExp n vtable a_reg
      ...
      *)
      ...
      let loop_main   = [Store elem_size (a_reg,addr_reg,0) ]
      let loop_footer = [ ADDI (addr_reg, addr_reg, 4)
                        ; ADDI (i_reg, i_reg, 1)
                        ; J loop_beg
                        ; LABEL loop_end
  ...
  *)
  | Filter (farg, arr, ret_type, pos) ->
      let arr_reg  = newReg "arr"   (* address of array *)
      let size_reg = newReg "size"  (* size of input array *)
      let elem_reg = newReg "elem" (* address of current element *)
      let addr_reg = newReg "elem" (* address of current element *)
      let i_reg    = newReg "ind_var"   (*new array counter *)
      let tmp_reg  = newReg "tmp"   (* several purposes *)
      let func_res = newReg "fresult"
      let loop_beg = newLab "loop_beg"
      let loop_end = newLab "loop_end"
      let isFalse = newLab "isFalse"
      let counter_reg = newReg "new_ind_var" (* address of element in new array *)


      let arr_code = compileExp arr vtable arr_reg
      let header1 = [LW(size_reg, arr_reg, 0)]
      let init_regs: Instruction list = [ ADDI (addr_reg, place, 4)
                      ;MV (i_reg, Rzero)
      ...
      let loop_code = 
        ...
        @ applyFunArg(farg, [tmp_reg], vtable, func_res, pos)
        @
        [BEQ(func_res,Rzero, isFalse)
        ;Store elem_size (tmp_reg,addr_reg,0)
      ...
      arr_code @ header1 @dynalloc(size_reg, place, ret_type) @init_regs  @ loop_code @ loop_end 
  ...
  *)
  ...
  | Scan (farg, ne_exp, arr_exp, ret_type, pos) ->
      let arr_reg  = newReg "arr"   (* address of array *)
      let size_reg = newReg "size"  (* size of input array *)
      let elem_reg = newReg "elem" (* address of current element *)
      let acc_reg = newReg "accumulator"  (*Accumulator*)
      let i_reg    = newReg "ind_var"   (* loop counter *)
      let tmp_reg  = newReg "tmp"   (* several purposes *)
      let loop_beg = newLab "loop_beg"
      let loop_end = newLab "loop_end"
      ...
      (* Compile initial value into place (will be updated below) *)
      let acc_code = compileExp ne_exp vtable acc_reg
      (* Set arr_reg to address of first element instead. *)
      (* Set i_reg to 0. While i < size_reg, loop. *)
      let elem_size = getElemSize ret_type
      let loop_code = 
      ...
      let loop_footer = [
        ...
        ; J loop_beg
        ; LABEL loop_end]
      arr_code @ header1 @dynalloc(size_reg, place, ret_type) @init_regs @ acc_code @ loop_code @ loop_footer 
