    ...
    match e with
        ...
        | Var (name, pos) ->
            ...
            *)
            ...
            match SymTab.lookup name vtable with
                | Some (ConstProp tmpVal) -> Constant(tmpVal, pos)
                | Some (VarProp tmpString) -> Var(tmpString, pos)
                | None -> e
        ...
        | Index (name, e, t, pos) ->
            ...
            *)
            ...
            let e1 = copyConstPropFoldExp vtable e
            match SymTab.lookup name vtable with
                | Some (ConstProp tmpVal) -> failwith "imp"
                | Some (VarProp tmpString) -> Index(tmpString, e1, t, pos)
                | None -> Index(name, e1, t, pos)
            ...
            match e' with
                | Var (a, _) ->
                    ...
                    *)
                    ...
                    let vtable1 = SymTab.bind name (VarProp a) vtable
                    let body1 = copyConstPropFoldExp vtable1 body
                    Let (Dec (name, e', decpos), body1, pos)
                ...
                | Constant (a, _) ->
                    ...
                    *)
                    ...
                    let vtable1 = SymTab.bind name (ConstProp a) vtable
                    let body1 = copyConstPropFoldExp vtable1 body
                    Let (Dec (name, e', decpos), body1, pos)
