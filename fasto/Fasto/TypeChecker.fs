...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    | Not (e, pos) ->
        let (t1,t2) = checkExp ftab vtab e
        (Bool,Not(t2,pos))
    ...
    *)
    | Replicate (n, a, ret_type, pos) ->
        ...
        let (a_type: Type, a_dec  ) = checkExp ftab vtab a
        match n_type with
          | Int -> (Array a_type, Replicate (n_dec, a_dec, a_type, pos))
    ...
    *)
    | Filter (f, arr, _, pos) ->
      let (arr_type, arr_exp_dec) = checkExp ftab vtab arr
      let elem_type =
          match arr_type with
            | Array t -> t
      ...
      if Bool <> f_res_type then
        reportTypesDifferent "return types not bool in Filter"
                             f_arg_type elem_type pos
    ...
    *)
    | Scan (f, ne, arr, _, pos) -> 
        let (n_type, n_dec) = checkExp ftab vtab ne
        let (arr_type, arr_dec) = checkExp ftab vtab arr
        let elem_type =
          match arr_type with
            | Array t -> t
            ...
            | (f', res, [a1; a2]) -> 
              if a1 <> a2 then
                reportTypesDifferent "argument types of operation in scan"
                                     a1 a2 pos
              if res <> a1 then
                reportTypesDifferent "argument and return type of opeartion in reduce"
                                     a1 res pos
              (f', res)
            | (_, res, args) ->
        ...
        if elem_type <> f_arg_type then
          reportTypesDifferent "operation and array-element types in scan"
                               f_arg_type elem_type pos
        if n_type <> f_arg_type then
          reportTypesDifferent "operation and start-element types in scan"
                               f_arg_type n_type pos
