...
let rec removeDeadBindingsInExp (e : TypedExp) : (bool * DBRtab * TypedExp) =
    match e with
        ...
        | Index (name, e, t, pos) ->
            ...
            *)
            let (hasIO, uses, e') = removeDeadBindingsInExp e
            (hasIO, recordUse name uses, Index(name, e', t, pos))
        ...
        | Let (Dec (name, e, decpos), body, pos) ->
            ...
            *)
            ...
            let (eHasIO, eUses, e') = removeDeadBindingsInExp e
            let (bodyHasIO, bodyUses, body') = removeDeadBindingsInExp body
            if (isUsed name bodyUses) || eHasIO
            then (eHasIO || bodyHasIO, SymTab.combine eUses bodyUses, Let (Dec (name, e', decpos), body', pos))
            else (bodyHasIO, bodyUses, body')
