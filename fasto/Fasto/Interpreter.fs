...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  *)
  ...
  | Divide(e1, e2, pos) ->
        ...
        let res2   = evalExp(e2, vtab, ftab)
        match (res1, res2) with
          | (IntVal n1, IntVal n2) -> try IntVal (n1/n2) with | :? DivideByZeroException -> raise (MyError("Attempted to divide by zero at: ", pos))
          | (IntVal _, _) -> reportWrongType "right operand of /" Int res2 (expPos e2)
          | (_, _) -> reportWrongType "left operand of /" Int res1 (expPos e1)
  | And (e1, e2, pos) ->
        let res1   = evalExp(e1, vtab, ftab)
  ...
  | Not(e, pos) ->
        ...
        | _ -> reportWrongType "operand of not" Bool res pos
  | Negate(e, pos) ->
        let res = evalExp(e, vtab, ftab)
        match res with
        | IntVal i -> IntVal (-i)
        | _ -> reportWrongType "operand of negate" Int res pos
  | Equal(e1, e2, pos) ->
  ...
  *)
  | Replicate (n, a, _, pos) ->
        let nAmount = evalExp(n, vtab, ftab)
        let aInitVal = evalExp(a, vtab, ftab)
        match nAmount with
          | IntVal n1 when n1 >= 0 ->
            let lst = List.replicate n1 aInitVal
            let tp1 = valueType aInitVal
            ArrayVal(lst, tp1)
          | otherwise -> reportWrongType "argument of \"Replicate\"" Int nAmount pos
  ...
  *)
  | Filter (farg, arrexp, _, pos) ->
        ...
        match arr with
          | ArrayVal (lst,tp1) ->
              let trueList = List.filter (fun x ->
                                            let return1 = evalFunArg (farg, vtab, ftab, pos, [x])
                                            match return1 with
                                              | BoolVal true -> true
                                              | _ -> false
                                          ) lst
              ArrayVal (trueList, tp1)
  ...
  *)
  ...
  (*
    - farg: is a function-pointer to a function that scan will use on the elements of the given array
    - ne: is an accumulator that is used during to keep track of progress
    - arrexp: is an expression that denotes an array of some kind ({1, 2, 3} for example)
    - pos: not to sure what this does
  *)
  (*
  ...
  *)
  | Scan (farg, ne, arrexp, _, pos) ->
      ...
      match arr with
        | ArrayVal (lst, tp1) ->
            ArrayVal (List.tail (List.scan (fun acc (x: Value) -> evalFunArg (farg, vtab, ftab, pos, [acc;x])) nel lst), farg_ret_type)
        | otherwise -> reportNonArray "3rd argument of \"reduce\"" arr pos
